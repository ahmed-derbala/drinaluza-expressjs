		...
		try {
			...
			return jwt.verify(token, config.auth.jwt.privateKey, async (err, decoded) => {
				...
				if (params.roles && params.roles.length > 0) {
					...
					const hasRole = req.user.roles.some((role) => params.roles.includes(role))
					if (!hasRole) {
						return resp({
							message: `Access denied for user ${req.user.slug} with roles: ${req.user.roles || 'none'}. Required roles: ${params.roles.join(', ')}`,
							status: 403,
							data: null,
							req,
							res
