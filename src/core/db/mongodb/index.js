	...
	mongoose.connection
		...
		.on('disconnected', () => {
			log({
				...
				level: config.log.levelNames.warn,label:"db-mongo"
			})
		})
		.on('disconnected', () => {
			log({
				message: 'db-conn-disconnected',
				level: config.log.levelNames.error,label:"db-mongo"
			})
		})
		.on('reconnected', () => {
			log({
				message: 'db-conn-reconnected',
				level: config.log.levelNames.verbose,label:"db-mongo"
			})
		})
		.on('fullsetup', () => {
			log({
