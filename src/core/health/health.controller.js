...
import { getPublicSocket, getPrivateSocket, getSocket } from '#socketio'
...
router.route('/').get(async (req, res) => {
	try {
		...
		const allClientsCount = getSocket().clientsCount

		const data = { node, app, uptime, socketio: { allClientsCount, publicClientsCount, privateClientsCount } }
